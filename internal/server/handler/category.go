...
func (h *CategoryHandler) Register(rg *gin.RouterGroup) {
	...
	g.POST("/create", h.CreateCategory)  // 新增品类
	g.POST("/get", h.GetCategory)        // 按 id 获取
	g.POST("/list", h.ListCategories)    // 列表（分页/条件）
	g.POST("/update", h.UpdateCategory)  // 更新品类
	g.POST("/delete", h.DeleteCategory)  // 删除品类
}

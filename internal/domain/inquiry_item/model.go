...
type InquiryItem struct {
	...
	SpecNameSnap         *string    `gorm:"column:spec_name_snap;size:32;comment:规格名称快照（如：新鲜/500g）"`
	UnitNameSnap         *string    `gorm:"column:unit_name_snap;size:32;comment:单位名称快照（如：斤/公斤）"`
	...
	CurrentAvgPrice      *float64   `gorm:"column:current_avg_price;type:decimal(12,2);comment:本期均价（可由市场报价汇总后回填）"`
	Sort                 int        `gorm:"not null;default:0;comment:排序码（保留与导出一致顺序）"`
...
func (i *InquiryItem) BeforeCreate(tx *gorm.DB) error {
	...
	if i.GoodsNameSnap == "" {
		return errors.New("GoodsNameSnap(goods_name_snap) 不能为空")
	}
	if i.CategoryNameSnap == "" {
		return errors.New("CategoryNameSnap(category_name_snap) 不能为空")
	}
	return nil
}

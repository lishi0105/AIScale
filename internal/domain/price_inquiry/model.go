...
type PriceInquiry struct {
	...
	OrgID         string    `gorm:"column:org_id;type:char(36);not null;comment:中队ID（base_org.id）"`
	InquiryTitle  string    `gorm:"column:inquiry_title;size:64;not null;comment:询价单标题"`
	InquiryDate   time.Time `gorm:"column:inquiry_date;type:date;not null;comment:业务日期（取当月任意一天）"`
	InquiryYear   int16     `gorm:"column:inquiry_year;type:smallint;comment:年份（生成列）"`
	InquiryMonth  int8      `gorm:"column:inquiry_month;type:tinyint;comment:月份（生成列）"`
	InquiryTenDay int8      `gorm:"column:inquiry_ten_day;type:tinyint;comment:旬：1=上旬 2=中旬 3=下旬（生成列）"`

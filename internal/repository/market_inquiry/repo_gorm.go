...
func (r *marketInquiryRepo) BatchCreateMarketInquiries(ctx context.Context, inquiries []domain.MarketInquiry) error {
	if len(inquiries) == 0 {
		return nil
	}
	return r.db.WithContext(ctx).CreateInBatches(inquiries, 100).Error
}

func (r *marketInquiryRepo) GetByItemAndMarket(ctx context.Context, itemID, marketName string) (*domain.MarketInquiry, error) {
	var out domain.MarketInquiry
	err := r.db.WithContext(ctx).
		Where("item_id = ? AND market_name_snap = ?", itemID, marketName).
		First(&out).Error
	if err != nil {
		return nil, err
	}

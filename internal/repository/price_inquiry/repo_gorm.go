...
func (r *priceInquiryRepo) ListPriceInquiries(ctx context.Context, params ListParams) ([]domain.PriceInquiry, int64, error) {
	...
	if params.InquiryYear != nil {
		q = q.Where("inquiry_year = ?", *params.InquiryYear)
	}
	if params.InquiryMonth != nil {
		q = q.Where("inquiry_month = ?", *params.InquiryMonth)
	}
	if params.InquiryTenDay != nil {
		q = q.Where("inquiry_ten_day = ?", *params.InquiryTenDay)
	}
